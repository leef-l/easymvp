...
按当前钱学森总纲，这个“实时项目驾驶舱”必须再补 3 个硬约束：

1. 驾驶舱主入口优先展示闭环对象，而不是旧对象链的自然语言摘要
2. 页面不能只显示运行活动，还必须显示阻塞原因、升级对象和人工检查点
3. `Workspace / Acceptance / Diagnostics` 的主消费对象已发生收口，不能继续只围绕 `AcceptanceRun / production_passed`
...
上面这组对象现在更适合作为历史背景与解释层。

按当前 EasyMVP 页面口径，页面真正优先消费的闭环对象应当是：
...
2. `CompiledTask / brain-v3 Run / RuntimeEscalation / CompletionVerdict` 进入 `Workspace`
3. `verification_contract_json / VerificationResult / CompletionVerdict` 进入 `Acceptance`
4. `FaultSummary / RepairPlanDraft` 进入 `Diagnostics` 或返工相关视图
5. 用户人工确认和放行动作进入 `ActionInbox`
...
如果未来扩展多人协作，也应作为后续专题处理，而不能回退为传统后台结构。
...
7. `manual_checkpoint_required`
8. `verification_conflict`
...
5. `VerificationResult / CompletionVerdict` 的变化
6. `RuntimeEscalation / FaultSummary` 的变化
...
1. 缺的是哪项 `required_check`
2. 缺的是哪项 `required_evidence`
3. 当前推荐验证通道是什么
...
1. `verification_contract` 摘要
2. check / evidence 覆盖图区
3. evidence 卡片区
4. `VerificationResult` 摘要
5. `CompletionVerdict` 裁决区
6. 必要时的 `RuntimeEscalation`

### 11.2 最终裁决区

裁决区必须明确区分：

1. `VerificationResult.verdict`
2. `CompletionVerdict.decision`
3. `CompletionVerdict.completed`
...
13. `has_runtime_escalation`
14. `has_verification_conflict`
