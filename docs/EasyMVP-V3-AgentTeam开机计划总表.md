...
| P-BR-002 | domain-brain | 落 `easymvp-brain` 调用服务与本地/远程统一客户端 | P0 | P-BR-001,P-RT-001 | 否 | `docs/EasyMVP-V3专精大脑接入计划.md#7. 正式调用链`；`docs/EasyMVP-V3-easymvp-brain-Manifest与ToolSchema设计.md` | `internal/service/easymvp_brain.go` 可按统一合同调用本地 sidecar 或远程服务，且三类 P0 合同具备 typed wrapper | in_progress |
| P-BR-003 | domain-brain | 接通 `plan_review` / `plan_compile` / `acceptance_mapping` 三个 P0 合同 | P0 | P-BR-002,P-DM-002 | 否 | `docs/EasyMVP-V3-easymvp-brain职责边界与输入输出合同设计.md`；`docs/EasyMVP-V3方案编译模型设计.md` | 可生成并落库 `PlanReviewResult`、`CompiledPlan`、`AcceptanceProfile` / `ProductionAcceptanceProfile` | in_progress |
| P-BR-004 | domain-brain | 接通 `completion_adjudication` / `repair_design` | P1 | P-BR-003,P-RT-002 | 是 | `docs/EasyMVP-V3-ProductionAcceptanceProfile证据结构与裁决规则.md`；`docs/EasyMVP-V3-easymvp-brain职责边界与输入输出合同设计.md` | 可生成 completion decision 与 repair draft，失败进入 `ActionInbox` | pending |
| P-BR-005 | domain-brain | 接通 `workspace_explanation` 推荐动作链路 | P1 | P-BR-003,P-DM-003,P-RT-003 | 是 | `docs/EasyMVP-V3-easymvp-brain-Prompt设计.md`；`docs/EasyMVP-V3工作台视图模型与聚合接口设计.md` | 工作台可读取 explanation headline、summary、recommended actions | pending |
...
| P-FE-001 | frontend-workbench | 初始化 Electron + React 桌面壳 | P0 | 无 | 是 | `docs/EasyMVP-V3-Electron-Go单仓脚手架与开发命令设计.md` | `apps/desktop` 可启动空壳 | in_progress |
| P-FE-002 | frontend-workbench | 落地 Workspace Home 页面骨架与状态组件 | P0 | P-FE-001,P-DM-003 | 是 | `docs/EasyMVP-V3-页面组件实现终稿与代码骨架规范.md` | Workspace 首页可渲染 loading/empty/error/normal | pending |
| P-FE-003 | frontend-workbench | 落地 Project Workspace 页面骨架 | P1 | P-FE-001,P-DM-003,P-RT-003 | 是 | `docs/EasyMVP-V3-页面组件实现终稿与代码骨架规范.md` | 单项目工作台页面可渲染 | pending |
| P-FE-004 | frontend-workbench | 接入领域脑 explain / acceptance / plan 真实交互 | P1 | P-BR-003,P-BR-005,P-FE-002,P-FE-003 | 是 | `docs/EasyMVP-V3-Plan详细页面设计.md`；`docs/EasyMVP-V3-Acceptance详细页面设计.md` | 前端能读取 plan/acceptance/explanation 真实结果并稳定展示 | pending |
...
2. 当前真正缺的是 `easymvp-brain` 领域合同实现层
3. 不补这层，EasyMVP 仍然只有查询和 runtime 接入，没有领域脑主链路
4. `brain-v3` 内置脑已经增强，若不先锁死协作边界，后续容易把 `code / browser / verifier / fault` 能力错误并回领域脑
...
2. `P-FE-001`
3. `P-DM-003` 的回归维护
4. `P-RT-002` / `P-RT-003` 的回归维护
5. `P-BR-006`

串行关键路径：

1. `P-BR-001 -> P-BR-002 -> P-BR-003`
2. `P-BR-003 -> P-BR-004`
3. `P-BR-003 -> P-BR-005 -> P-FE-004`
