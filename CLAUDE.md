...
**任何操作都必须优先保护服务器资源，不能为了推进任务牺牲运行稳定性。**

### 核心原则

1. **先看负载再执行**：开始任何可能消耗资源的操作前，先检查当前服务器负载。
...
5. **测试和编译统一交给 GitHub Actions**：未获人工明确许可时，严禁在当前机器直接运行任何测试或编译命令；其中 `go test` 为明确禁令，默认一律禁止执行。其他禁止命令包括 `go build`、`npm/pnpm test`、`pnpm exec vue-tsc`、`pnpm build`、`pnpm exec vite build`、`docker build` 等。需要验证时，只能修改或触发 `.github/workflows/`，并以 GitHub Actions 结果为准。
...
## 十八、铁律：按照优先级完整推进，实现闭环

**所有工作必须按优先级向前推进，不能停留在分析、局部修改或半完成状态；只要条件允许，就必须做到账务闭环式交付。**

### 核心原则

1. **先主线后支线**：优先处理阻塞主流程、影响上线、影响正确性的事项，再处理体验优化、文档润色、局部重构等次级事项。
2. **先完成再扩展**：同一事项必须先做到“可运行、可验证、可交付”，再继续做额外优化；禁止把核心任务做成半截工程。
3. **默认推进到闭环**：除非人工明确要求只分析、只设计、只给方案，否则默认应继续推进到实现、校验路径明确、结果可交付。
4. **发现问题要连续处理**：实现过程中发现缺失项、阻塞项、配套改动、必要文档或配置遗漏时，应一并补齐，避免留下明显断点。
5. **无法闭环时必须明确卡点**：如果确实受限于权限、环境、外部依赖或人工决策，必须明确说明当前卡点、影响范围、下一步动作，不能以“已分析”或“已部分完成”作为结束。

### 执行口径

- 排序默认遵循：正确性 > 阻塞主流程 > 安全与数据权限 > 数据库迁移与兼容性 > 用户可见功能 > 性能优化 > 代码洁癖式整理。
- 回应任务时，默认以“定位问题 → 实施修改 → 补齐配套项 → 通过 GitHub Actions 验证 → 反馈结果”作为完整链路。
- 如果任务涉及多个子问题，应先完成最关键的可交付闭环，再继续处理次优先级项；不要平均用力。
- 若人工重新指定优先级，始终以人工指令为最高优先级。
