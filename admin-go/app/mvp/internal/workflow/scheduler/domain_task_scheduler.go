...
var listReadyPendingSyncByWorkflowFn = func(ctx context.Context, workflowRunID int64) ([]g.Map, error) {
	return repo.NewTaskWorkspaceRepo().ListReadyPendingSyncByWorkflow(ctx, workflowRunID, nil)
}
...
func (s *DomainTaskScheduler) OnTaskCompleted(ctx context.Context, taskID int64) error {
	...
	task, err := s.taskRepo.GetByIDMap(ctx, taskID, "workflow_run_id")
	...
	if workflowRunID == 0 {
		return fmt.Errorf("任务 %d 不存在或缺少 workflow_run_id", taskID)
	}
	s.publishTaskEvent(ctx, workflowRunID, taskID, event.EventTaskCompleted, nil)
	...
	if s.publisher == nil {
		go func() {
			...
			s.ReconcileWorkflowProgress(context.Background(), workflowRunID)
		}()
	}
	return nil
}
...
func (s *DomainTaskScheduler) OnTaskFailed(ctx context.Context, taskID int64, errMsg string) {
	...
	s.publishTaskEvent(ctx, workflowRunID, taskID, event.EventTaskFailed, payload)
	go func() {
		defer func() {
			if r := recover(); r != nil {
		...
		if s.onTaskFailed != nil && s.onTaskFailed(context.Background(), workflowRunID, taskID, errMsg) {
			return
		}
		s.scheduleOnce(context.Background(), workflowRunID)
	}()
}
...
func (s *DomainTaskScheduler) scheduleOnce(ctx context.Context, workflowRunID int64) {
	if s.hasBlockingPendingDeliveries(ctx, workflowRunID) {
		return
	}
	...
	batches := []int{0}
	if batchNo > 0 {
		batches = append(batches, batchNo)
	}
	tasks, err := s.taskRepo.ListByWorkflowBatchesAndStatusesOrdered(
		ctx,
		workflowRunID,
		batches,
		[]string{domainTask.StatusPending},
		"id", "batch_no", "sort", "affected_resources", "depends_on_task_ids", "parent_task_id",
	)
	...
	for _, task := range tasks {
		...
		resJSON := g.NewVar(task["affected_resources"]).String()
		...
		now := gtime.Now()
		casRows, casErr := s.taskRepo.UpdateFieldsIfStatus(ctx, taskID, domainTask.StatusPending, g.Map{
		...
		if len(resources) > 0 {
			...
			if jsonErr != nil {
				...
				if upErr := s.taskRepo.UpdateFields(ctx, taskID, g.Map{"locked_resources": string(lockedJSON)}); upErr != nil {
...
func (s *DomainTaskScheduler) getActiveBatch(ctx context.Context, workflowRunID int64) int {
	...
	batchNo, err := s.taskRepo.GetMinBatchNoByWorkflowStatusGreaterThan(ctx, workflowRunID, domainTask.StatusPending, 0)
	if err != nil || batchNo == 0 {
		return 0
	}

	// 检查该批次之前是否有未完成的任务（非 completed/skipped）
	prevUnfinished, puErr := s.taskRepo.CountByWorkflowBatchLessThanNotInStatuses(ctx, workflowRunID, batchNo, []string{domainTask.StatusCompleted, "skipped"})
	if puErr != nil {
	...
	if prevUnfinished > 0 {
		// 之前批次还有未完成的，不推进
		prevBatch, prevErr := s.taskRepo.GetMinBatchNoByWorkflowBatchLessThanNotInStatuses(ctx, workflowRunID, batchNo, []string{domainTask.StatusCompleted, "skipped"})
		if prevErr != nil {
			g.Log().Warningf(ctx, "[DomainTaskScheduler] 查询前序最小未完成批次失败: wfRunID=%d err=%v", workflowRunID, prevErr)
			return 0
		}
		return prevBatch
	}
...
func (s *DomainTaskScheduler) releaseTaskResources(taskID int64) {
	...
	// DB 清理
	if upErr := s.taskRepo.UpdateFields(context.Background(), taskID, g.Map{"locked_resources": nil, "heartbeat_at": nil}); upErr != nil {
...
func (s *DomainTaskScheduler) checkAllDone(ctx context.Context, workflowRunID int64) {
	...
	unfinished, ufErr := s.taskRepo.CountByWorkflowNotInStatuses(ctx, workflowRunID, []string{domainTask.StatusCompleted, "skipped", domainTask.StatusEscalated})
	if ufErr != nil {
	...
	if unfinished == 0 {
		...
		recheck, _ := s.taskRepo.CountByWorkflowNotInStatuses(ctx, workflowRunID, []string{domainTask.StatusCompleted, "skipped", domainTask.StatusEscalated})
		if recheck > 0 {
...
func (s *DomainTaskScheduler) shouldReconcileWorkflow(ctx context.Context, workflowRunID int64) bool {
	record, err := s.workflowRepo.GetByIDMap(ctx, workflowRunID, "status", "current_stage")
	if err != nil {
	...
	stage := g.NewVar(record["current_stage"]).String()
...
func (s *DomainTaskScheduler) HasUnfinished(ctx context.Context, workflowRunID int64) bool {
	...
	count, err := s.taskRepo.CountByWorkflowNotInStatuses(ctx, workflowRunID, []string{domainTask.StatusCompleted, "skipped", domainTask.StatusEscalated})
	if err != nil {
...
func (s *DomainTaskScheduler) hasBlockingPendingDeliveries(ctx context.Context, workflowRunID int64) bool {
	records, err := listReadyPendingSyncByWorkflowFn(ctx, workflowRunID)
	if err == nil {
		return len(records) > 0
	}

	lowerErr := strings.ToLower(err.Error())
	if strings.Contains(lowerErr, "unknown column") {
		return false
	}

	g.Log().Warningf(ctx, "[DomainTaskScheduler] 查询 blocking pending deliveries 失败: workflowRunID=%d err=%v", workflowRunID, err)
	return true
}
...
func (s *DomainTaskScheduler) Pause(ctx context.Context, workflowRunID int64) {
	...
	// 查 running 任务
	runningTasks, qErr := s.taskRepo.ListByWorkflowAndStatuses(ctx, workflowRunID, []string{domainTask.StatusRunning}, "id")
	if qErr != nil {
	...
	rows, pauseErr := s.taskRepo.UpdateByWorkflowAndStatuses(ctx, workflowRunID, []string{domainTask.StatusRunning}, g.Map{
...
func (s *DomainTaskScheduler) allDepsCompleted(ctx context.Context, task g.Map) bool {
	// 优先检查多依赖字段
	depsJSON := g.NewVar(task["depends_on_task_ids"]).String()
	if depsJSON != "" && depsJSON != "null" && depsJSON != "[]" {
		var depIDs []int64
		if err := json.Unmarshal([]byte(depsJSON), &depIDs); err == nil && len(depIDs) > 0 {
			// 批量查询所有依赖任务状态（避免 N+1）
			depStatuses, dsErr := s.taskRepo.ListByIDs(ctx, depIDs, "id", "status")
			if dsErr != nil {
			...
			for _, d := range depStatuses {
				statusMap[g.NewVar(d["id"]).Int64()] = g.NewVar(d["status"]).String()
			}
	...
	parentID := g.NewVar(task["parent_task_id"]).Int64()
	if parentID > 0 {
		parentTask, stErr := s.taskRepo.GetByIDMap(ctx, parentID, "status")
		if stErr != nil {
			g.Log().Warningf(ctx, "[DomainTaskScheduler] 查询父任务状态失败: parentID=%d err=%v", parentID, stErr)
			return false
		}
		st := g.NewVar(parentTask["status"]).String()
