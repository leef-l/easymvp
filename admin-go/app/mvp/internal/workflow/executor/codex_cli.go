...
func (e *CodexCLIExecutor) Execute(ctx context.Context, req *Request) *Result {
	// 加载引擎配置
	engineCfg, err := repo.NewAIEngineConfigRepo().GetEnabledByCode(ctx, "codex_cli")
	...
	if cmdTemplate != "" {
		...
		if req.ModelInfo != nil {
			...
			envVars["AI_MODEL_BASE_URL"] = resolveProtocolBaseURL(req.ModelInfo, g.NewVar(engineCfg["base_url"]).String(), "openai_compatible")
		}
	...
	if baseURL := resolveProtocolBaseURL(req.ModelInfo, g.NewVar(engineCfg["base_url"]).String(), "openai_compatible"); baseURL != "" {
