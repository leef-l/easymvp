...
func (e *GeminiCLIExecutor) Execute(ctx context.Context, req *Request) *Result {
	...
	if len(targets.DirectoryPaths) > 0 {
		if err := ensureDirectoryTargets(workDir, targets.DirectoryPaths); err != nil {
			if req.Workspace != nil && e.wsMgr != nil {
				finalizeWorkspaceFailure(ctx, e.wsMgr, req.TaskID, "GeminiCLIExecutor", err.Error(), false)
			}
	...
	if err != nil {
		...
		if req.Workspace != nil && e.wsMgr != nil {
			finalizeWorkspaceFailure(ctx, e.wsMgr, req.TaskID, "GeminiCLIExecutor", errMsg, false)
		}
