...
type MvpProjectCategory struct {
	...
	VerificationProfileJson string      `orm:"verification_profile_json" description:"分类默认验证配置(JSON)"` // 分类默认验证配置(JSON)
	VerificationGateJson    string      `orm:"verification_gate_json" description:"分类验证放行规则(JSON)"`    // 分类验证放行规则(JSON)
