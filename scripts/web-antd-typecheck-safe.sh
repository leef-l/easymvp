...
MIN_AVAILABLE_MB="${EASYMVP_TYPECHECK_MIN_AVAILABLE_MB:-1024}"
...
if (( HEAP_MB >= MEMORY_LIMIT_MB )); then
  echo "EASYMVP_TYPECHECK_HEAP_MB must be smaller than EASYMVP_PNPM_MEMORY_MB (${HEAP_MB} >= ${MEMORY_LIMIT_MB})" >&2
  exit 2
fi
...
if (( available_mb < MIN_AVAILABLE_MB )); then
  echo "Skip ${TYPECHECK_LABEL}: available memory ${available_mb}MB < required ${MIN_AVAILABLE_MB}MB" >&2
  exit 3
fi
...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "Another ${TYPECHECK_LABEL} is already running: $LOCK_FILE" >&2
  exit 4
fi

echo "Running ${TYPECHECK_LABEL} on current server"
echo "  root: $ROOT_DIR"
...
if (( status == 137 || status == 143 )); then
  echo "${TYPECHECK_LABEL} was terminated under the 1 CPU / 1G guard (exit=${status})" >&2
fi
